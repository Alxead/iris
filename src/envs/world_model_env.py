...
class BlockCausalWorldModelEnv:
    ...
    def reset_from_initial_observations(self, observations: torch.FloatTensor) -> torch.FloatTensor:
        ...
        self.create_empty_keys_values(obs_tokens.size(0))
        self.obs_tokens = obs_tokens

        return self.decode_obs_tokens()

    @torch.no_grad()
    def create_empty_keys_values(self, n):
    ...
    def step(self, action: Union[int, np.ndarray, torch.LongTensor], should_predict_next_obs: bool = True) -> None:
        ...
        if self.keys_values_wm.size + (1 + self.num_observations_tokens) > self.world_model.config.max_tokens:
            self.create_empty_keys_values(self.obs_tokens.size(0))
        ...
        prev_tokens = torch.cat((self.obs_tokens, action_token), dim=1)      # (B, K+1)

        outputs_wm = self.world_model(prev_tokens, past_keys_values=self.keys_values_wm)

        reward = Categorical(logits=outputs_wm.logits_rewards).sample().float().cpu().numpy().reshape(-1) - 1  # (B,)
        done = Categorical(logits=outputs_wm.logits_ends).sample().cpu().numpy().astype(bool).reshape(-1)      # (B,)
        self.obs_tokens = Categorical(logits=outputs_wm.logits_observations).sample()  # (B, K)
